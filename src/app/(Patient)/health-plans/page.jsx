            ...
            <div className='my-container'>
              ...
              {healthPlans.map((hp, ind)=>{
                const {planName, description, price, features} = hp
                    return <div key={ind} className='rounded space-y-4 bg-slate-200 text-black border-2 border-red-500 p-5 relative pb-20'>
                        <h2 className='my-subtitle'>{planName}</h2>
                        <h3 className='my-title'>৳ {price} <span className='!text-lg font-semibold'>/months</span></h3>
                        <p className='text-slate-700'> {description}</p>
                        <ul className='space-y-3'> 
                        {features?.map((feature, ind)=><li key={ind} className='flex items-center gap-2'> <FaCheck></FaCheck> {feature}</li> )}
                        </ul>
                        <button className='my-btn-one-outline absolute bottom-2 left-0 right-0 mx-5'>Subscribe Now</button>
                    </div>
                })}
              </div>
            </div>
